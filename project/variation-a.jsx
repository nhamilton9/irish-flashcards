...
function VarA_Setup({ mode, state, onStart, onBack }) {
  ...
  const [band, setBand] = React.useState(0); // 0..9 -> 1..100, 101..200 etc.
  const start = band * 100;
  const end = start + 100;

  // How many of these 100 are already seen / known?
  ...
  return (
    <VarA_Shell>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "4px 22px 22px", maxWidth: 640, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
        {/* Session size */}
        <div style={{ background: "#fff", border: `1px solid ${A.parchment2}`, borderRadius: 18, padding: "18px 20px", marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 14 }}>
            <div>
              <span className="serif" style={{ fontSize: 36, color: A.mossDeep, fontWeight: 500, lineHeight: 1 }}>{size}</span>
          ...
          />
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: A.stone, marginTop: 4 }}>
        ...
        <div style={{ background: "#fff", border: `1px solid ${A.parchment2}`, borderRadius: 18, padding: "18px 20px", marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 10 }}>
            <span className="serif" style={{ fontSize: 18, color: A.mossDeep, fontWeight: 500 }}>
              Raon {rangeLabel(start, end)}
            </span>
            <span style={{ fontSize: 10, color: A.stone, letterSpacing: 1, textTransform: "uppercase" }}>Range</span>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 5 }}>
            {Array.from({ length: 10 }, (_, i) => (
              <button key={i} className="btn" onClick={() => setBand(i)} style={{
                padding: "10px 0",
                background: band === i ? A.mossDeep : "transparent",
                border: band === i ? "none" : `1px solid ${A.parchment2}`,
                borderRadius: 8, cursor: "pointer",
                color: band === i ? A.parchment : A.stoneDark,
                fontFamily: "'Karla', sans-serif", fontSize: 11,
                fontWeight: band === i ? 600 : 500,
          ...
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: `1px solid ${A.parchment2}`,
            display: "flex", justifyContent: "space-between", fontSize: 11, color: A.stoneDark }}>
            <span><b style={{ color: A.ink }}>{bandStats.seen}</b>/100 feicthe</span>
            <span><b style={{ color: A.moss }}>{bandStats.known}</b>/100 eolach</span>
          </div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 22px 28px", maxWidth: 640, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
        <button className="btn" onClick={() => onStart(mode, size, start, end)} style={{
          ...
          Tosaigh · Start {size} {focalForm(size)}
        </button>
      </div>
    </VarA_Shell>
  );
...
function VarA_Flashcard({ initialDeck, onFinish, onBack, state, rateWord }) {
  ...
  if (!card) return null;
  const cardHeight = wide ? 420 : 360;
  const frontSize = wide ? 72 : 56;
  ...
  return (
    <VarA_Shell>
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px 24px 10px" }}>
      ...
      <div style={{ padding: "8px 22px 28px", flexShrink: 0, maxWidth: 640, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
        {!flipped ? (
          <div style={{ textAlign: "center", fontSize: 11, color: A.stone, paddingTop: 8 }}>
            Nocht an freagra chun é a rátáil · {wide ? "Space to flip" : "tap to reveal"}
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 7 }}>
            <button className="btn" onClick={() => rate("again")} style={rateBtnA(A.rust)}>
              <div style={{ fontSize: 10, opacity: 0.7 }}>Níl a fhios {wide && <span style={{ opacity: 0.6 }}>· 1</span>}</div>
              <div style={{ fontSize: 12, fontWeight: 600 }}>Don't know</div>
            </button>
            <button className="btn" onClick={() => rate("hard")} style={rateBtnA(A.peat)}>
              <div style={{ fontSize: 10, opacity: 0.7 }}>Deacair {wide && <span style={{ opacity: 0.6 }}>· 2</span>}</div>
              <div style={{ fontSize: 12, fontWeight: 600 }}>Hard</div>
            </button>
            <button className="btn" onClick={() => rate("easy")} style={rateBtnA(A.moss)}>
              <div style={{ fontSize: 10, opacity: 0.7 }}>Éasca {wide && <span style={{ opacity: 0.6 }}>· 3</span>}</div>
              <div style={{ fontSize: 12, fontWeight: 600 }}>Easy</div>
            </button>
          </div>
        )}
      </div>
...
const rateBtnA = (bg) => ({
  background: bg, color: A.parchment, border: "none",
