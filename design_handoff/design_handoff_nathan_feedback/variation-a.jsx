...
function VarA_Header({ title, subtitle, streak, onBack, right, accent }) {
  const { wide } = React.useContext(LayoutCtx);
  const padTop = wide ? 28 : 56;
  const titleColor = accent || A.mossDeep;
  return (
    <div style={{ padding: `${padTop}px 22px 12px`, flexShrink: 0, maxWidth: 960, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: subtitle ? 10 : 0 }}>
        ...
        <div className="uncial" style={{ fontSize: wide ? 22 : 18, color: titleColor, letterSpacing: 1 }}>
          {title}
        </div>
...
function VarA_Home({ stats, state, onOpenSetup, onShowList }) {
  ...
  const dens = density === "compact" ? { padX: 16, gap: 6, btnPad: "11px 14px", cardPad: "12px 14px", ringMargin: 4, statsMargin: 2 }
             : density === "spacious" ? { padX: 26, gap: 10, btnPad: "16px 20px", cardPad: "20px 22px", ringMargin: 12, statsMargin: 8 }
             : { padX: 22, gap: 8, btnPad: "14px 18px", cardPad: "18px 20px", ringMargin: 8, statsMargin: 4 };
  const ringSize = wide ? 220 : (t.ringSize || 128);
  const accent = t.accent || A.mossDeep;

  return (
    <VarA_Shell>
      <VarA_Header title="Na Mílte Focal" subtitle={t.showSubtitle ? "The Thousand Words" : null} streak={streak} accent={accent} />
      ...
      <div style={{
        ...
        {/* Progress ring + stats */}
        <div style={{ padding: wide ? 0 : `${dens.ringMargin}px ${dens.padX}px ${dens.statsMargin}px`, textAlign: "center", flex: wide ? "0 0 320px" : "none", flexShrink: 0 }}>
          <div style={{ position: "relative", width: ringSize, height: ringSize, margin: `${dens.ringMargin}px auto ${density === 'compact' ? 4 : 10}px` }}>
            <div style={{ position: "absolute", inset: 0, borderRadius: "50%", border: `2px solid ${A.stone}`, opacity: 0.25 }} />
            <div style={{ position: "absolute", inset: 10, borderRadius: "50%", border: `1px solid ${accent}`, opacity: 0.15 }} />
            <div style={{
            ...
            <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
              <div className="serif" style={{ fontSize: Math.round(ringSize * 0.3), lineHeight: 1, color: accent, fontWeight: 500 }}>
                {stats.pct}<span style={{ fontSize: Math.round(ringSize * 0.12), color: A.stone }}>%</span>
              </div>
              <div style={{ fontSize: 9, color: A.stone, letterSpacing: 1.5, textTransform: "uppercase", marginTop: 3 }}>
                ar eolas
              </div>
            </div>
          </div>
        ...
        {/* Main actions */}
        <div style={{ padding: wide ? 0 : `4px ${dens.padX}px ${dens.padX}px`, flex: 1, width: "100%", minHeight: 0 }}>
          <div style={{
            background: "#fff", border: `1px solid ${A.parchment2}`,
            borderRadius: 18, padding: dens.cardPad, marginBottom: 10,
            boxShadow: `0 1px 0 ${A.parchment2}`,
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: density === 'compact' ? 10 : 16 }}>
              <span className="uncial" style={{ fontSize: 15, color: accent }}>Tosaigh Seisiún</span>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: dens.gap }}>
              <button className="btn" onClick={() => onOpenSetup("flashcard")} style={stackedBtnA(true, accent, dens.btnPad)}>
                <span>
              ...
              <button className="btn" onClick={() => onOpenSetup("quiz-ga-en")} style={stackedBtnA(false, accent, dens.btnPad)}>
              ...
              <button className="btn" onClick={() => onOpenSetup("quiz-en-ga")} style={stackedBtnA(false, accent, dens.btnPad)}>
                <span className="serif" style={{ fontSize: 15, color: A.ink }}>
          ...
          <button className="btn" onClick={onShowList} style={{
            ...
            borderRadius: 14, padding: dens.btnPad,
...
const stackedBtnA = (primary, accent, pad) => ({
  width: "100%",
  display: "flex", alignItems: "center", justifyContent: "space-between",
  background: primary ? (accent || A.mossDeep) : A.parchment,
  border: primary ? "none" : `1px solid ${A.parchment2}`,
  color: primary ? A.parchment : A.ink,
  borderRadius: 12, padding: pad || "14px 18px",
  fontFamily: "'Karla', sans-serif", fontWeight: 500,
  cursor: "pointer", textAlign: "left",
});

// --- SETUP ------------------------------------------------------------------
// Pick size + range before starting. Range = 100-word bands (1–100, 101–200, …)
// plus broader groupings (1–500, 501–1000, All).
function VarA_Setup({ mode, state, onStart, onBack }) {
  ...
  // How many of the selected range are already seen / known?
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "4px 22px 16px", maxWidth: 640, width: "100%", margin: "0 auto", boxSizing: "border-box" }}>
        ...
        <div style={{ background: "#fff", border: `1px solid ${A.parchment2}`, borderRadius: 18, padding: "16px 18px", marginBottom: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 5 }}>
            <button className="btn" onClick={() => setRangeKey("h1")} style={rangePill(rangeKey === "h1", true)}>1–500</button>
            <button className="btn" onClick={() => setRangeKey("h2")} style={rangePill(rangeKey === "h2", true)}>501–1000</button>
            <button className="btn" onClick={() => setRangeKey("all")} style={rangePill(rangeKey === "all", true)}>Uile · All</button>
          </div>
        ...
        {/* Start button — inline, not pushed to bottom, so the spacing feels balanced */}
...
const rangePill = (active, wide) => ({
  padding: wide ? "9px 0" : "10px 0",
...
function VarA_Flashcard({ initialDeck, onFinish, onBack, state, rateWord }) {
  ...
  const [rated, setRated] = React.useState([]); // first-rating log per unique card
  ...
  if (!card) return null;
  const cardHeight = wide ? 420 : (t.cardHeight || 320);
  const frontSize = wide ? 72 : (t.frontSize || 52);

  // Ireland flag rating colors — green / white / orange
  ...
  return (
    <VarA_Shell>
      ...
      {/* Card area — stable vertical position. The rating buttons row is
          reserved even when not flipped (a transparent placeholder of the
          same height) so the card never jumps when you flip it. */}
      <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "16px 24px 18px", minHeight: 0 }}>
        <div className="flip-card" style={{ width: "100%", maxWidth: 560, height: cardHeight }}>
          <div className={`flip-inner ${flipped ? "flipped" : ""}`} onClick={() => !editingEx && setFlipped(!flipped)}>
            ...
            <div className="flip-face flip-back" style={{
              background: accent, color: A.parchment,
              ...
              <div style={{
                ...
                {editingEx ? (
                  ...
                  <div
                    onClick={() => { setExDraft(customEx || ""); setEditingEx(true); }}
                    style={{ cursor: "text" }}
        ...
        {/* Rating row — always present; invisible when not flipped so the card
            stays put. */}
...
function VariationA() {
  ...
  let body = null;
  if (screen === "home") body = <VarA_Home stats={stats} state={state} onOpenSetup={openSetup} onShowList={() => setScreen("list")} />;
  else if (screen === "setup") body = <VarA_Setup mode={setupMode} state={state} onStart={onStart} onBack={() => setScreen("home")} />;
  else if (screen === "list") body = <VarA_List state={state} onBack={() => setScreen("home")} />;
  else if (screen === "flashcard") body = <VarA_Flashcard initialDeck={deck} state={state} rateWord={rateWord} onFinish={onFinish} onBack={() => setScreen("home")} />;
  else if (screen === "quiz-ga-en") body = <VarA_Quiz initialDeck={deck} dir="ga-en" rateWord={rateWord} onFinish={onFinish} onBack={() => setScreen("home")} />;
  else if (screen === "quiz-en-ga") body = <VarA_Quiz initialDeck={deck} dir="en-ga" rateWord={rateWord} onFinish={onFinish} onBack={() => setScreen("home")} />;
  else if (screen === "results") body = <VarA_Results rated={rated} streakInfo={state.streak} onHome={() => setScreen("home")} onAgain={() => onStart(mode, deck.length, range.start, range.end)} />;

  return (
    <TweaksCtx.Provider value={t}>
      {body}
      <TweaksPanel>
        <TweakSection label="Home screen density" />
        <TweakRadio label="Density" value={t.density} options={["compact","regular","spacious"]}
          onChange={(v) => setTweak("density", v)} />
        <TweakSlider label="Ring size" value={t.ringSize} min={80} max={200} step={4} unit="px"
          onChange={(v) => setTweak("ringSize", v)} />
        <TweakToggle label="Show stats row" value={t.showStats}
          onChange={(v) => setTweak("showStats", v)} />
        <TweakToggle label="Show subtitle" value={t.showSubtitle}
          onChange={(v) => setTweak("showSubtitle", v)} />
        <TweakSection label="Flashcard" />
        <TweakSlider label="Card height" value={t.cardHeight} min={240} max={440} step={10} unit="px"
          onChange={(v) => setTweak("cardHeight", v)} />
        <TweakSlider label="Front font" value={t.frontSize} min={32} max={80} step={2} unit="px"
          onChange={(v) => setTweak("frontSize", v)} />
